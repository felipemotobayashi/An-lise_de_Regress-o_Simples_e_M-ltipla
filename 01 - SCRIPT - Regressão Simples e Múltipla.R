...
my_plot2 <-
  ...
  geom_smooth(aes(color = "Fitted Values OLS"),
              method = "lm", formula = y ~ x, se = F, size = 1.5) +
...
my_plot2
ggdraw() +
  draw_image("https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQhX6yk4PuUseDOFGctqr6E2t0osNNefMaceA&usqp=CAU",
             x = -0.23, y = 0.19, scale = .25) +
  draw_image("https://icones.pro/wp-content/uploads/2021/08/icone-x-avec-cercle-orange.png",
             x = 0.02, y = -0.162, scale = .23) +
  draw_plot(my_plot2)
beep(1) #função do pacote 'beepr'
...
#Gráfico de dispersão com emoji
ggplotly(
  bebes %>%
...
#Para calcular o lambda de Box-Cox
#função 'powerTransform' do pacote 'car'
lambda_BC <- powerTransform(empresas$retorno)
lambda_BC
